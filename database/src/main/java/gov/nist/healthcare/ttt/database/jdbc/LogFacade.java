...
//        LogFacade lf = new LogFacade(this.getConfig());
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            String input = XDRFacade.readFile("/home/mccaffrey/working/badsql2.txt", Charset.defaultCharset());
            report.setValidationReport(input);

            lf.addNewCCDAValidationReport("1073cb15-5de5-458a-998b-e36c173abbad", report);
            ...
            /*
             ...
             String input = XDRFacade.readFile("/home/mccaffrey/working/badsql.txt", Charset.defaultCharset());
             ...
             PartInterface part = new PartImpl();
             String rawMessage = "This is the raw message with \\'.";
             part.setRawMessage(rawMessage);
             lf.addNewPart("1073cb15-5de5-458a-998b-e36c173abbad", part);

             PartInterface get = lf.getPartByMessageId("1073cb15-5de5-458a-998b-e36c173abbad");
             String getRawMessage = get.getRawMessage();
             ...
             LogImpl log = new LogImpl();
             log.setIncoming(Boolean.TRUE);
             log.setTimestamp(Long.toString(Calendar.getInstance().getTimeInMillis()));
             log.setMessageId(UUID.randomUUID().toString());
             log.setMdn(Boolean.TRUE);
             ArrayList from = new ArrayList();
             from.add("hello@world.com");
             from.add("hello2@world.com");
             log.setFromLine(from);
             log.setStatus(Status.ERROR);
             log.setOriginalMessageId("origmsgid");
             lf.addNewLog(log);
             }  
            
            
             Collection<LogInterface> logs =   lf.getIncomingByFromLine("hello@world.com");
             System.out.println(logs.size() + " = size!");
             //  Collection<LogInterface> logs = lf.getOutgoingByToLine("hello2@world.com");
             //  System.out.println(logs.size() + " = size!");
             //    
             ...
             //  lf.updateStatus("111b", Status.MDN_RECEIVED);
             // System.out.println(lf.getLogByMessageId("111b").getOriginalMessageId() + "hello");

             /*Collection<Log> log2 = lf.getOutgoingByToLine("hello@world.com");

             Iterator<Log> it = log2.iterator();
             while (it.hasNext()) {
             System.out.println(it.next().getLogID());
             }
             */
            /*
             PartImpl part = new PartImpl();
             part.setRawMessage("rawmessagehere");
             DetailImpl detail1 = new DetailImpl();
             DetailImpl detail2 = new DetailImpl();
             detail1.setName("detail1");
             detail2.setName("detail2");
             Collection<DetailInterface> details = new ArrayList<DetailInterface>();
             details.add(detail1);
             details.add(detail2);
             part.setDetails(details);

             PartImpl child = new PartImpl();
             child.setRawMessage("childMessage");
             Collection<PartInterface> children = new ArrayList<PartInterface>();
             children.add(child);

             part.setChildren(children);

             lf.addNewPart("111b", part);
             
             */

            /*  PartInterface part = lf.getPartByMessageId("111b");
             ...
             while(it.hasNext()) {
             PartImpl child = (PartImpl) it.next();
             System.out.println(child.getPartID() + " " + child.getRawMessage());
             }
            
             Collection<DetailInterface> details = part.getDetails();
             Iterator<DetailInterface> it2 = details.iterator();
             while(it2.hasNext()) {
             DetailInterface detail = it2.next();
